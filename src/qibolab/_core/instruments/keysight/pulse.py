...
def generate_qcs_envelope(shape: Envelope, duration: float) -> qcs.Envelope:
...
def process_acquisition_channel_pulse(
    ...
    Arguments:
        ...
        time_of_flight (float): Digitizer pre-delay to sync readout DAC and ADC
    """
    ...
    if pulse.kind == "delay":
        qcs_pulse = qcs.Delay(
            sweep_param_map.get(
                "duration",
                np.round(pulse.duration * NS_TO_S * SR) / SR,
        ...
        qcs_pulse = qcs.RFWaveform(
            duration=sweep_param_map.get(
                "duration",
                np.round(pulse.probe.duration * NS_TO_S * SR) / SR,
        ...
        program.add_acquisition(
            ...
            # pre_delay=time_of_flight * NS_TO_S if time_of_flight != 0 else None
        )
